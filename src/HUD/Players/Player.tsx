...
export default class PlayerBox extends React.Component<IProps> {
	render() {
		...
		const primary =
			weapons.filter((weapon) => !['C4', 'Pistol', 'Knife', 'Grenade', undefined].includes(weapon.type))[0] ||
			null;
		...
		return (
			<div className={`player ${player.state.health === 0 ? 'dead' : ''} ${isObserved ? 'active' : ''}`}>
				<div className='player_data'>
					{player.avatar ? (
						<Avatar steamid={player.steamid} height={57} showSkull={player.state.health === 0} />
					) : (
					...
					<div className='player_stats'>
						<div className='row'>
							...
							<div className='username'>
								<div>
									{isLeft ? <span>{player.observer_slot}</span> : null} {player.name}{' '}
									{!isLeft ? <span>{player.observer_slot}</span> : null}
								</div>
								{primary || secondary ? (
									<Weapon
										weapon={primary ? primary.name : secondary.name}
										active={primary ? primary.state === 'active' : secondary.state === 'active'}
									/>
								) : (
									''
								)}
								{player.state.round_kills ? (
									<div className='roundkills-container'>{player.state.round_kills}</div>
								) : null}
							</div>
						</div>
						<div className='row'>
							...
							<div className='secondary_weapon'>
								{primary && secondary ? (
									<Weapon weapon={secondary.name} active={secondary.state === 'active'} />
								) : (
									''
								)}
							</div>
